class AcGameMenu {
	constructor(root){
		this.root = root; // root是页面中的ac_game
		this.$menu = $(`
		...
		`);
		this.$menu.hide();
		this.root.$ac_game.append(this.$menu); // 将menu里面渲染的html加到ac_game指向的id里面
...
let AC_GAME_ANIMATION = function(timestamp){
	for(let i = 0;i < AC_GAME_OBJECTS.length;i++){
		...
		if(!obj.has_called_start){
			obj.start(); // 没有执行过start，执行一次tart
			obj.has_called_start = true;
		} else {
			obj.timedelta = timestamp - last_timestamp;
			obj.update(); // 确保每一帧执行一次
		}
	}
...
class NoticeBoard extends AcGameObject { // 记分牌
    constructor(playground) {
        super();
...
class Settings {
				...
				this.$settings = $(`
					<div class="ac-game-settings">
						<div class="ac-game-settings-login">
							...
							<!--	<img width="30" src="https://app3866.acapp.acwing.com.cn/static/image/settings/acwing_logo.png"> -->
								<br>
								<div>
									QQ登录
								</div>
		...
		login_on_remote() { // 在远程服务器上登录
			...
			$.ajax({
				url: "https://app3866.acapp.acwing.com.cn/settings/login/",
				type: "GET",
				data: {
					username: username,
					password: password,
		...
		register_on_remote(){ // 在远程服务器上注册
			...
			this.$register_error_message.empty();// 清空error_message
			$.ajax({
				url: "https://app3866.acapp.acwing.com.cn/settings/register/",
				type: "GET",
				data: {
					username: username,
					password: password,
		...
		logout_on_remote() { // 在远程服务器上登出
			if(this.platform === "ACAPP"){
				...
				$.ajax({
					url: "https://app3866.acapp.acwing.com.cn/settings/logout/",
					type: "GET",
					success: function(resp){
		...
		getinfo() {
				let outer = this;
				$.ajax({
					url:"https://app3866.acapp.acwing.com.cn/settings/getinfo/",
					type: "GET",
					data:{
						platform: outer.platform,
					},
					success: function(resp) { // resp是用户后端传来的JSON信息
